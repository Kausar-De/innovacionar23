...
from userint.models import Participant, Question
...
def entryPage(request):
    ...
    aftertime = datetime.datetime(2022, 4, 10, hour = 18, minute = 0, second = 0)
...
def extract_CSV(request):
    ...
    writer.writerow(['Participant ID', 'Team Name', 'Team Lead Name', 'Team Lead Phone No.', 'Team Lead Email', 'Member 2 Name', 'Member 3 Name', 'Member 4 Name', 'Submission Link', 'Creation Date'])

    for parti in Participant.objects.all().values_list('participant', 'teamname', 'teamlead', 'leadphone', 'leademail', 'member2', 'member3', 'member4', 'submissionlink', 'created_date'):
        writer.writerow(parti)

    response['Content-Disposition'] = 'attachment; filename = "AppstrixSubmissions.csv"'
